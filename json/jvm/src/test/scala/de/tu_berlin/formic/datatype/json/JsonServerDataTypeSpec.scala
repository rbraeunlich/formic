...
class JsonServerDataTypeSpec extends TestKit(ActorSystem("TreeServerDataTypeSpec"))
  ...
  "A Json server data type" must {
    ...
    "insert data" in {
      ...
      json ! OperationMessage(ClientId(), dataTypeInstanceId, JsonServerDataTypeFactory.name, List(op2))
      json ! OperationMessage(ClientId(), dataTypeInstanceId, JsonServerDataTypeFactory.name, List(op3))
    ...
    "delete data" in {
      ...
      json ! OperationMessage(ClientId(), dataTypeInstanceId, JsonServerDataTypeFactory.name, List(deletion))
    ...
    "replace data" in {
      ...
      json ! OperationMessage(ClientId(), dataTypeInstanceId, JsonServerDataTypeFactory.name, List(replacementOp))
    ...
    "result in valid JSON representation" in {
      ...
      json ! OperationMessage(ClientId(), dataTypeInstanceId, JsonServerDataTypeFactory.name, List(op))
      json ! UpdateRequest(ClientId(), dataTypeInstanceId)
      expectMsg(UpdateResponse(dataTypeInstanceId, JsonServerDataTypeFactory.name, "{\"node\":{\"arr\":[2],\"bool\":true,\"num\":1,\"str\":\"ab\"}}", Some(op.id)))
    }
  }
