...
class JsonTransformer extends TreeTransformer {
  ...
  private def transform(op1: JsonReplaceOperation, op2: JsonReplaceOperation, context: OperationContext): DataStructureOperation = {
  ...
  private def transform(op1: JsonReplaceOperation, op2: TreeInsertOperation, context: OperationContext): DataStructureOperation = {
  ...
  private def transform(op1: JsonReplaceOperation, op2: TreeDeleteOperation, context: OperationContext): DataStructureOperation = {
    if (isEffectIndependent(op1.accessPath, op2.accessPath)) {
      return JsonReplaceOperation(op1.accessPath, op1.tree, op1.id, context, op1.clientId)
    }
    val tp = transformationPoint(op1.accessPath, op2.accessPath)
